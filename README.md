# qwq AIOps 平台

<div align="center">

**AI 驱动的智能运维平台**

[![License](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)
[![Go Version](https://img.shields.io/badge/Go-1.23+-00ADD8?logo=go)](https://go.dev/)
[![Vue Version](https://img.shields.io/badge/Vue-3.x-4FC08D?logo=vue.js)](https://vuejs.org/)
[![Docker](https://img.shields.io/badge/Docker-20.10+-2496ED?logo=docker)](https://www.docker.com/)

[English](README_EN.md) | 简体中文

</div>

## ✨ 特性

- 🤖 **AI 智能诊断** - 基于 OpenAI/Ollama 的智能问题分析和解决方案推荐
- 📊 **实时监控** - 系统资源、服务状态、容器健康度实时监控
- 🔔 **智能告警** - 自动巡检异常并通过钉钉/企业微信/邮件推送
- 🐳 **容器管理** - Docker 容器的启动、停止、重启和日志查看
- 📈 **可视化面板** - Prometheus + Grafana 监控可视化
- 🔐 **安全可靠** - JWT 认证、数据加密、权限控制
- 🚀 **一键部署** - Docker Compose 一键部署，开箱即用

## 🎯 快速开始

### 前置要求

- Docker 20.10+
- Docker Compose V2
- 4GB+ 可用内存
- 10GB+ 可用磁盘空间

### 一键部署

```bash
# 1. 克隆项目
git clone https://github.com/QwQBiG/qwq-aiops.git
cd qwq-aiops

# 2. 配置 AI 服务（必须）
cp .env.example .env
nano .env  # 编辑配置文件

# 3. 运行部署脚本
chmod +x deploy.sh
./deploy.sh

# 或使用简化脚本
chmod +x start.sh
./start.sh
```

**Windows 用户**:
```bash
start.bat
```

### AI 配置

qwq 是 AI 驱动的平台，必须配置 AI 服务才能使用。

#### 方式 1: OpenAI API（推荐新手）

编辑 `.env` 文件：

```bash
AI_PROVIDER=openai
OPENAI_API_KEY=sk-your-api-key-here
OPENAI_BASE_URL=https://api.openai.com/v1
OPENAI_MODEL=gpt-3.5-turbo
```

#### 方式 2: Ollama 本地模型（推荐企业）

```bash
# 1. 安装 Ollama
curl -fsSL https://ollama.com/install.sh | sh

# 2. 下载模型
ollama pull qwen2.5:7b

# 3. 编辑 .env
AI_PROVIDER=ollama
OLLAMA_HOST=http://host.docker.internal:11434
OLLAMA_MODEL=qwen2.5:7b
```

### 访问系统

部署成功后，访问以下地址：

| 服务 | 地址 | 说明 |
|------|------|------|
| 前端界面 | http://localhost:8081 | 主界面 |
| API 文档 | http://localhost:8081/api/docs | Swagger 文档 |
| Prometheus | http://localhost:9091 | 监控指标 |
| Grafana | http://localhost:3000 | 可视化面板 |

**默认账号**:
- 用户名: `admin`
- 密码: `admin123`

## 📖 文档

- [快速开始](快速开始.md) - 5 分钟快速上手
- [部署指南](docs/deployment-guide.md) - 详细部署说明
- [用户手册](docs/user-manual.md) - 功能使用说明
- [API 文档](http://localhost:8081/api/docs) - RESTful API 文档
- [常见问题](docs/faq.md) - 常见问题解答

## 🏗️ 架构

```
                    ┌─────────────────────────────┐
                    │      前端 (Vue 3)            │
                    │  监控面板  | 容器管理         │
                    │  AI  诊断 | 系统设置          │
                    └─────────────┬───────────────┘
                                  │
                                  ▼
                    ┌─────────────────────────────┐
                    │     后端 API (Go)            │
                    │  Web 服务 | AI Agent         │
                    │  监控引擎 | 告警推送          │
                    └─────────────┬───────────────┘
                                  │
            ┌─────────────────────┼─────────────────────┐
            ▼                     ▼                     ▼
    ┌──────────────┐      ┌──────────────┐    ┌──────────────┐
    │    MySQL     │      │    Redis     │    │  Prometheus  │
    │   数据存储    │      │   缓存服务    │    │   监控指标    │
    └──────────────┘      └──────────────┘    └──────────────┘
```

## 🔧 技术栈

### 后端
- **语言**: Go 1.23+
- **框架**: Cobra (CLI), Gorilla (WebSocket)
- **AI**: OpenAI API / Ollama
- **数据库**: SQLite / MySQL / PostgreSQL
- **缓存**: Redis
- **监控**: Prometheus

### 前端
- **框架**: Vue 3 + Vite
- **UI**: Element Plus
- **图表**: ECharts
- **编辑器**: Monaco Editor
- **状态管理**: Pinia

### 运维
- **容器**: Docker + Docker Compose
- **反向代理**: Nginx (可选)
- **监控**: Prometheus + Grafana

## 📊 功能模块

### 1. 智能诊断
- AI 驱动的问题分析
- 自动生成解决方案
- 命令执行建议
- 知识库集成

### 2. 系统监控
- CPU、内存、磁盘实时监控
- 负载、网络连接监控
- 自定义监控规则
- 历史数据查询

### 3. 容器管理
- Docker 容器列表
- 容器启动/停止/重启
- 实时日志查看
- 容器资源监控

### 4. 告警推送
- 钉钉机器人
- 企业微信机器人
- 邮件通知
- Slack 集成

### 5. 自动巡检
- 定时系统巡检
- 异常自动检测
- AI 分析报告
- 健康日报推送

## 🚀 部署方式

### Docker Compose（推荐）

```bash
# 快速启动
./deploy.sh

# 或手动启动
docker compose up -d --build
```

### 手动部署

```bash
# 1. 构建前端
cd frontend
npm install
npm run build

# 2. 构建后端
cd ..
go build -o qwq ./cmd/qwq/main.go

# 3. 运行
./qwq web
```

### Kubernetes

```bash
# 使用 Helm Chart
helm install qwq ./charts/qwq

# 或使用 kubectl
kubectl apply -f k8s/
```

## ⚙️ 配置说明

### 环境变量

| 变量名 | 说明 | 默认值 | 必填 |
|--------|------|--------|------|
| `PORT` | 服务端口 | 8080 | 否 |
| `AI_PROVIDER` | AI 提供商 (openai/ollama) | - | 是 |
| `OPENAI_API_KEY` | OpenAI API Key | - | 条件 |
| `OLLAMA_HOST` | Ollama 服务地址 | http://localhost:11434 | 条件 |
| `DB_TYPE` | 数据库类型 | sqlite | 否 |
| `JWT_SECRET` | JWT 密钥 | - | 是 |

完整配置请参考 [.env.example](.env.example)

### 端口说明

| 端口 | 服务 | 说明 |
|------|------|------|
| 8081 | qwq 主服务 | 前端 + API |
| 3308 | MySQL | 数据库 |
| 6380 | Redis | 缓存 |
| 9091 | Prometheus | 监控 |
| 3000 | Grafana | 可视化 |

## 🔒 安全建议

### 生产环境部署

1. **修改默认密码**
   ```bash
   # 修改 .env 中的密钥
   JWT_SECRET=$(openssl rand -base64 32)
   ENCRYPTION_KEY=$(openssl rand -base64 32)
   ```

2. **启用 HTTPS**
   ```bash
   # 使用 Nginx 反向代理
   # 配置 SSL 证书
   ```

3. **配置防火墙**
   ```bash
   # 只开放必要端口
   ufw allow 80/tcp
   ufw allow 443/tcp
   ```

4. **定期备份**
   ```bash
   # 配置自动备份
   BACKUP_ENABLED=true
   BACKUP_SCHEDULE="0 2 * * *"
   ```

## 🤝 贡献

欢迎贡献代码、报告问题或提出建议！

1. Fork 本仓库
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启 Pull Request

## 📝 更新日志

### v1.0.0 (2024-12-08)

- ✨ 初始版本发布
- 🤖 集成 OpenAI 和 Ollama AI 支持
- 📊 完整的监控和告警系统
- 🐳 Docker 容器管理
- 📈 Prometheus + Grafana 集成
- 🔐 JWT 认证和权限控制

## 📄 许可证

本项目采用 MIT 许可证 - 详见 [LICENSE](LICENSE) 文件

## 🙏 致谢

- [OpenAI](https://openai.com/) - AI 能力支持
- [Ollama](https://ollama.com/) - 本地 AI 模型
- [Vue.js](https://vuejs.org/) - 前端框架
- [Go](https://go.dev/) - 后端语言
- [Docker](https://www.docker.com/) - 容器化
- [Prometheus](https://prometheus.io/) - 监控系统

## 📞 联系方式

- 项目主页: https://github.com/QwQBiG/qwq-aiops
- 问题反馈: https://github.com/QwQBiG/qwq-aiops/issues
- 文档: https://github.com/QwQBiG/qwq-aiops/wiki

---

<div align="center">

**如果这个项目对你有帮助，请给一个 ⭐️ Star！**

Made with ❤️ by qwq Team

</div>
