...
<script setup>
// 文件管理视图组件 - 提供文件浏览、编辑、删除和目录管理功能
import { ref, computed, onMounted } from 'vue'
import axios from 'axios'
import { ElMessage, ElMessageBox } from 'element-plus'

// 响应式数据
const currentPath = ref('/')        // 当前浏览路径
const files = ref([])                // 文件列表
const loading = ref(false)           // 加载状态
const showEditor = ref(false)        // 编辑器弹窗显示状态
const showMkdir = ref(false)         // 新建目录弹窗显示状态
const currentFile = ref('')          // 当前编辑的文件路径
const fileContent = ref('')          // 文件内容
const saving = ref(false)            // 保存状态
const newDirName = ref('')           // 新建目录名称
